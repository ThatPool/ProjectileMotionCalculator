	...
	public void SetRigidbodyVariables(){
		...
		foreach (Transform child in transform){
			...
			if (child.name != "_Alphabets")
            {
				...
				thisRigidbody.drag = Drag;
				thisRigidbody.angularDrag = AngularDrag;
				thisRigidbody.useGravity = CollisionEvent ? false : UseGravity;
				thisRigidbody.isKinematic = CollisionEvent ? true : IsKinematic;
                    if (CollisionEvent)
                    {
                        if (child.gameObject.GetComponent<OnCollisionGravity>() == null)
                        {
							child.gameObject.AddComponent<OnCollisionGravity>();
                        }
                    }
                    else
                    {
						DestroyImmediate(child.gameObject.GetComponent<OnCollisionGravity>());
                    }
